    ...
    yield* Match.value(cmd).pipe(
        ...
        state.phase !== "auth"
          ...
          : Effect.gen(function* () {
              ...
              yield* pipe(
                ...
                Effect.tapErrorTag("ProtocolError", (e) => Effect.logWarning("auth failed", { cause: e })),
      ...
      Match.tag("DataEnd", () =>
        Effect.gen(function* () {
          ...
          const envelope = current.envelope ?? { from: "" as Email, to: [] as ReadonlyArray<Email> };
...
const handleSession = (socket: Socket.Socket) =>
  Effect.gen(function* () {
    ...
    yield* Effect.forever(Effect.flatMap(takeLine, (line) => processLine(line, ref, write, maxDataBytes))).pipe(
      Effect.catchTag("SessionDone", () => Effect.void),
    );
  }).pipe(Effect.scoped);
