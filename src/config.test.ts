...
describe("Config trust boundary", () => {
  it("should decode valid XhConfig from Effect default export", () =>
    Effect.gen(function* () {
      ...
      expect(result.accounts[0]?.email).toBe("a@b.com");
      expect(result.accounts[0]?.password).toBe("secret");
  ...
  it("should allow partial server overrides", () =>
    Effect.gen(function* () {
      ...
      expect(result.server.hostname).toBe("mail.example.com");
  ...
  it("should share caller's fiber tree for Config access", () =>
    Effect.gen(function* () {
      ...
      expect(result.accounts).toHaveLength(1);
      expect(result.accounts[0]?.email).toBe("test_value@test.com");
    }).pipe(
