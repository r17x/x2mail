      ...
      send: (raw, envelope) =>
        Effect.scoped(
          Effect.gen(function* () {
            ...
            return { messageId: (idMatch?.[1] ?? `smtp-${DateTime.toEpochMillis(now)}`) as MessageId };
          }),
        ).pipe(
          Effect.tapErrorTag("SocketError", (e) => Effect.logWarning("smtp relay socket error", { cause: e })),
