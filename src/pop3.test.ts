...
import type { Email, Hostname, MessageId, MsgNum } from "./schema.ts";
import { MailStore, layer as mailStoreLayer } from "./store.ts";

const testServerConfig = Layer.succeed(ServerConfig, ServerConfig.of({
...
describe("POP3 Response Builders", () => {
  it("should produce greeting", () => {
    expect(Pop3Cmd.greeting()).toBe("+OK xh2m POP3 server ready\r\n");
  });
...
describe("MailStore", () => {
  ...
  it("should isolate messages between accounts", () =>
    Effect.gen(function* () {
      const store = yield* MailStore;
      yield* store.addMessages("alice@test.com" as Email, [makeTestMessage("msg-a", "Alice's mail")]);
