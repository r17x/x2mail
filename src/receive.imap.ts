...
const makeSession = (config: {
  ...
  Effect.gen(function* () {
    ...
    const socket = yield* makeSocket(config);
    ...
    return { sendCommand, readUntilTagged };
  });

const withSession = <A>(
  config: { host: string; port: number; username: string; password: string; tls: boolean },
  body: (session: {
  ...
  Effect.scoped(
    Effect.gen(function* () {
      const { sendCommand, readUntilTagged } = yield* makeSession(config);
    ...
    Effect.tapErrorTag("SocketError", (e) => Effect.logWarning("imap socket error", { cause: e })),
...
export const make = (config: {
  ...
  Layer.succeed(
    ...
    ReceiveProvider.of({
      fetch: (since) =>
        withSession(config, ({ sendCommand, readUntilTagged }) =>
          Effect.gen(function* () {
            ...
            return yield* Effect.forEach(messageNums, (num) =>
              Effect.gen(function* () {
                ...
                const contentLines = fetchResults.length > 1
                  ? fetchResults.slice(1, fetchResults[fetchResults.length - 1] === ")" ? -1 : undefined)
                  : [];
