...
import type { PostmarkSendConfig } from "./schema.ts";
import { SendProvider } from "./send.ts";

export const make = (config: typeof PostmarkSendConfig.Type) =>
  Layer.effect(
    SendProvider,
    Effect.gen(function* () {
      ...
      return SendProvider.of({
        send: Effect.fn("SendProvider.send.postmark")(
          function* (raw, _envelope) {
            yield* postmark.post("https://api.postmarkapp.com/email/rawMessage", {
          ...
          (E) =>
            E.pipe(
              Effect.tapErrorTag("HttpClientError", (e) =>
                Effect.logWarning("postmark API error", { cause: e }),
              ),
